...
class FastScanEvalCaseTest(unittest.TestCase):
    ...
    def test_real_controller_runs_five_fixtures_and_negative_stays_zero_call(self):
        ...
        graph_cases = json.loads(
            (ROOT / "evals/graph-cases.json").read_text(encoding="utf-8")
        )["cases"]
        ...
        for policy, graph_case in zip(policies[:5], graph_cases[:5]):
            with tempfile.TemporaryDirectory() as directory:
                ...
                (root / ".requirements-impact-refiner.json").write_text(
                    json.dumps({"impact_graph": {
                        "enabled": True, "max_seconds": 30,
                        "target_seconds": 10, "providers": ["builtin"],
                ...
                result = controller.scan_impact(controller.ScanRequest(
                    root, graph_case["request"],
                    tuple(graph_case["repository_evidence"]), "balanced",
                ))
                wrapper_path = (
                    root / ".requirements-impact-refiner/scans" /
                    (result.scan_id + ".json")
                )
                ...
                exact_provenance = (
                    ...
                    and wrapper["graph_receipt"]["providers"][0]["name"]
                    == "builtin"
                )
                ...
                observations.append(FastScanPerformanceObservation(
                    policy.id, 1, RunStatus.PASS, 1, None,
                    result.elapsed_ms, len(result.display_text.split()),
                    ("rir_scan",), True, exact_provenance, (), None, None,
                ))
        observations.append(FastScanPerformanceObservation(
            policies[-1].id, 1, RunStatus.PASS, 1, None, None, 0, (),
            True, True, (), None, None,
        ))
