...
from tests.test_validate_impact_report import PRE_DECISION_REPORT, VALID_REPORT, VALIDATOR
...
class SemanticValidationTest(unittest.TestCase):
    ...
    def test_rejects_missing_or_unknown_impact_classification(self):
        missing_category = VALID_REPORT.replace("| interfaces | critical |", "|  | critical |", 1)
        unknown_category = VALID_REPORT.replace(
            "| interfaces | critical |", "| contract | critical |", 1
        )
        missing_severity = VALID_REPORT.replace("| interfaces | critical |", "| interfaces |  |", 1)

        self.assert_rejected(missing_category, "impact IMP-001 requires a category")
        self.assert_rejected(unknown_category, "impact IMP-001 has invalid category contract")
        self.assert_rejected(missing_severity, "impact IMP-001 requires severity")
    ...
    def test_rejects_empty_behavior_and_evidence_basis(self):
        ...
        self.assert_rejected(empty_behavior, "invariant INV-001 requires a current behavior")
    ...
    def test_rejects_empty_preserved_invariant_content(self):
        ...
        self.assert_rejected(report, "preserved invariant INV-001 requires a requirement link")
    ...
    def test_every_active_impact_requires_acceptance_criteria(self):
        ...
        self.assert_rejected(report, "impact IMP-001 requires acceptance criteria")
    ...
    def test_rejects_empty_acceptance_observation_or_test(self):
        ...
        self.assert_rejected(empty_test, "criterion AC-001 requires evidence or test")
    ...
    def test_blocked_impact_keeps_planning_handoff_not_ready(self):
        ...
        self.assert_rejected(report, "blocked impacts require Planning Handoff workflow Not ready")
    ...
    def test_superpowers_marker_is_the_only_named_blocked_or_predecision_exception(self):
        ...
        predecision = PRE_DECISION_REPORT.replace(
            "| Not ready |", f"| {SUPERPOWERS_HANDOFF_MARKER} |", 1
        )
        ...
        arbitrary = PRE_DECISION_REPORT.replace("| Not ready |", "| superpowers |", 1)
        self.assert_rejected(arbitrary, "pre-decision Planning Handoff workflow must be Not ready")

    def test_accepted_and_deferred_impacts_remain_named_risks(self):
        report = VALID_REPORT.replace("| Accepted IMP-001 |", "| none |", 1)

        self.assert_rejected(report, "remaining risks must name accepted impact IMP-001")

    def test_planning_handoff_requires_every_field(self):
        report = VALID_REPORT.replace("| Existing planning workflow |", "|  |", 1)

        self.assert_rejected(report, "Planning Handoff requires Selected planning workflow")
