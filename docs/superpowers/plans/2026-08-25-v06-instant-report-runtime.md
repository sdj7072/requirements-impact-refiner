...
Use a 250ms operation deadline and bounded output for explicitly scoped Git probes. Only a matching v2 state/evidence/payload context, equal before/after HEAD/status/index snapshots, clean recursive submodules, and a finalize-time descriptor hash for every inventory or explicit request/evidence/seed path is `fresh`. Commit blobs are additionally checked only when no replacement refs or checkout-transforming attributes/config are present. Missing/slow Git, index races, unbound generated paths, divergent gitlinks, replacements, filters/encoding/EOL transforms, or incomplete proof is `stale`; missing context is `none`. Multiple matching lineages are `ambiguous` without body disclosure. Exact interrupted schema1 metadata/context may migrate to v2 only for the same draft, analysis, state, Markdown, and revision, with context verified before metadata replacement and tracked-source proof forced false; either interrupted ordering is recoverable.
