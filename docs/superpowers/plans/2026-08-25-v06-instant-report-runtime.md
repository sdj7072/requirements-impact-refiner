...
Use a 250ms operation deadline and bounded output for explicitly scoped Git probes. Only a matching current-schema/current-payload context, equal before/after HEAD/status/index/config/info-attributes snapshots, no gitlinks, and a finalize-time descriptor hash for every inventory or explicit request/evidence/seed path is `fresh`. Configured `core.autocrlf`, `core.eol`, or `core.attributesfile`; any `filter.*` configuration; a nonempty Git `info/attributes`; or a tracked root or nested `.gitattributes` makes proof incomplete. Missing/slow Git, index races, path-derivation overflow, an unresolved explicit path, any gitlink, replacement refs, transform state, or other incomplete proof is `stale`; missing context is `none`. Multiple matching lineages are `ambiguous` without body disclosure. Public v0.5 schema-1 completion metadata remains readable only for its lineage key map when beginning a later revision. Unreleased intermediate metadata/context shapes are never migrated or rebound; only an exact current-schema/current-payload retry resumes, and schema/payload mismatch fails closed without previous body disclosure.
