...
1. Use a fresh agent context for every repetition and supply only the case request and `repository_evidence`, never the rubric. For a multi-turn compact Codex case, retain the append-only report directory in the same isolated workspace; the resume prompt requires the exact bytes of canonical Markdown selected by `current.json` as predecessor bytes. `first.final.txt` remains chat continuity evidence and is the predecessor only for a legacy full-inline report with no persisted canonical artifact. This distinction does not expose a hidden rubric.
...
3. Human-adjudicate every `must_detect` and `must_not_do` entry for every repetition: the fixed matrix has 80 rubrics and requires 400 unique quoted rows. Pass `must_detect` only when output connects it to evidence or marks it inferred/unknown, and fail `must_not_do` when output performs the forbidden neighboring workflow. For `INT-superpowers`, mechanical scoring checks only the exact structured `Planning Handoff` marker; free-text `no automatic planning` semantics require a quoted human judgment. A `partial`, `blocked`, `infra_error`, or `invalid_evidence` result never counts as a pass.
