...
1. Use a fresh agent context for every repetition and supply only the case request and `repository_evidence`, never the rubric. For a multi-turn Codex case, retain the first final as `first.final.txt` in the same isolated workspace; the resume prompt identifies it as continuity evidence and requires predecessor hashes to use its exact bytes, but does not expose a hidden rubric.
