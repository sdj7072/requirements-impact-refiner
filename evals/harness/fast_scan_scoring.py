...
def load_fast_scan_cases(path=None):
    ...
    for raw in value["cases"]:
        if set(raw) != {"id", "kind", "required_seeds", "minimum_path_distance", "maximum_output_words", "maximum_scan_ms", "controller_required"}:
            raise ValueError("Fast Scan case fields are invalid")
        seeds = tuple((row["term"], row["location"]) for row in raw["required_seeds"])
        rows.append(FastScanCase(raw["id"], raw["kind"], seeds, raw["minimum_path_distance"], raw["maximum_output_words"], raw["maximum_scan_ms"], raw["controller_required"]))
    result = tuple(rows)
...
def score_fast_scan(case, result: Mapping[str, object]):
    ...
    if case.kind == "negative":
        if calls != []: findings.append("negative case must not call controller")
        if result.get("status") != "not_applicable": findings.append("negative case status is invalid")
    else:
        if calls != ["rir_scan"]: findings.append("Fast Scan must call only rir_scan once")
        if result.get("status") != "complete": findings.append("positive Fast Scan must complete")
        actual = {(row.get("term"), row.get("location")) for row in result.get("seeds", []) if isinstance(row, dict)}
        if not set(case.required_seeds).issubset(actual): findings.append("required seeds are missing")
        if result.get("maximum_path_distance", -1) < case.minimum_path_distance: findings.append("required distant path is missing")
        elapsed = result.get("elapsed_ms")
        if isinstance(elapsed, bool) or not isinstance(elapsed, int) or elapsed > case.maximum_scan_ms: findings.append("Fast Scan exceeds 30 seconds")
