...
_CASE_KEYS = frozenset((
    ...
    "forbidden_precision", "allowed_providers", "unknown_frontier_expected",
    "compact_output_phrases", "controller_required", "fixture_files",
))
...
def _case(raw) -> GraphCaseSpec:
    ...
    for node in raw["required_nodes"]:
        ...
        nodes.append(RequiredNode(
            ...
            node["kind"] if isinstance(node["kind"], str) and node["kind"] else (_ for _ in ()).throw(GraphCatalogError(f"{case_id} node kind is invalid")),
            node["label"] if isinstance(node["label"], str) and node["label"] else (_ for _ in ()).throw(GraphCatalogError(f"{case_id} node label is invalid")),
    ...
    for row in raw["forbidden_precision"]:
        ...
        forbidden.append(ForbiddenPrecision(
            row["provider"],
    ...
    for fixture in raw["fixture_files"]:
        if not isinstance(fixture, dict) or set(fixture) != {"path", "content"} or not isinstance(fixture["content"], str):
    ...
    result = GraphCaseSpec(
        case_id, kind, request, evidence, tuple(seeds), tuple(nodes),
        _strings(
            raw["required_edge_types"],
            f"{case_id} required edge types",
            unique=False,
        ),
        distance, tuple(forbidden),
    ...
    if kind == "negative" and any((
        result.seeds, result.required_nodes, result.required_edge_types,
        result.minimum_path_distance, result.forbidden_precision,
        result.allowed_providers, result.compact_output_phrases,
        result.controller_required, result.fixture_files,
    )):
...
def load_graph_cases(path: Optional[Path] = None) -> Tuple[GraphCaseSpec, ...]:
    selected = Path(path) if path is not None else Path(__file__).resolve().parents[1] / "graph-cases.json"
    try:
        payload = json.loads(selected.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError, RecursionError) as error:
        raise GraphCatalogError(f"cannot read graph catalog: {error}") from error
    if not isinstance(payload, dict) or set(payload) != {"schema_version", "cases"} or payload.get("schema_version") != 1 or not isinstance(payload.get("cases"), list):
    ...
    if selected.resolve() == (Path(__file__).resolve().parents[1] / "graph-cases.json").resolve() and ids != GRAPH_CASE_IDS:
...
def score_graph(case: GraphCaseSpec, receipt: Optional[Mapping[str, object]], final_output: str) -> GraphScore:
    ...
    if not all(isinstance(value, list) for value in (nodes_raw, edges_raw, paths_raw, frontier_raw, providers_raw)):
        return GraphScore(case.id, False, tuple(sorted(set(findings + ["receipt graph collections are invalid"]))), 0, receipt_id, digest, (), (), ())
    nodes = {row.get("id"): row for row in nodes_raw if isinstance(row, dict) and isinstance(row.get("id"), str)}
    edges = {row.get("id"): row for row in edges_raw if isinstance(row, dict) and isinstance(row.get("id"), str)}
    ...
    for row in nodes_raw:
        ...
        for forbidden in case.forbidden_precision:
            if row.get("provider") == forbidden.provider and row.get("confidence") in forbidden.confidences:
    ...
    for row in edges_raw:
        ...
        for forbidden in case.forbidden_precision:
            if row.get("provider") == forbidden.provider and (
                row.get("confidence") in forbidden.confidences or row.get("kind") in forbidden.edge_types
            ):
                findings.append("forbidden fabricated precision appears on graph evidence")
    ...
    covered = {identifier for path in paths_raw if isinstance(path, dict) for identifier in path.get("nodes", ())}
    frontier_nodes = {row.get("node") for row in frontier_raw if isinstance(row, dict)}
    uncovered = tuple(sorted(
        identifier for identifier, row in nodes.items()
        if set(row.get("risk_domains", ())) & HIGH_RISK_DOMAINS
        and identifier not in covered and identifier not in frontier_nodes
    ))
    if uncovered:
    ...
    matched_ids = tuple(
        path["id"] for path in matching_paths if isinstance(path.get("id"), str)
    )
    return GraphScore(
        case.id, not unique, unique, maximum_distance, receipt_id, digest,
        provider_names, uncovered, matched_ids,
    )
