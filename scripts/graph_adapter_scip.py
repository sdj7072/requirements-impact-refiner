...
class _ProviderContract(Protocol):
    ...
    def cleanup_private_root(
        self, directory: Path | str, directory_fd: int
    ) -> tuple[bool, bool, str | None]: ...

    def create_private_root(self, prefix: str = "rir-provider-data-") -> tuple[Path, int]: ...
...
class _CommonContract(Protocol):
    def _kind(self, path: object, label: str) -> str: ...

    def _regular_bytes(self, root: Path, relative: str) -> bytes | None: ...
...
def _is_provider_contract(value: object) -> TypeGuard[_ProviderContract]:
    return all(
        ...
        for name in ("cleanup_private_root", "create_private_root", "run_provider")
    )
...
def _read_exact_identity(descriptor, deadline, *, write_descriptor=None):
    ...
    if (
        ...
        or initial.st_size > _MAX_INDEX_BYTES
    ):
...
def _failure(status, detail, digests=()):
    return ProviderResult(
        "scip", status, "verified-provider", raw_receipt_sha256=digests, detail=str(detail)[:512]
    )
...
def _parse(value, root, fingerprint):
    if (
        ...
        or set(value) != {"schemaVersion", "metadata", "documents"}
    ...
    if not all(
        isinstance(indexer.get(key), str) and indexer[key] and len(indexer[key]) <= 128
    ...
    if (
        not isinstance(metadata["sourceFingerprint"], str)
        or _HEX64.fullmatch(metadata["sourceFingerprint"]) is None
    ):
    ...
    def finish(value):
        ...
        if not cleaned or changed:
            return (
                None,
                "unsafe",
                cleanup_detail or "private SCIP snapshot cleanup failed",
                before,
            )
        return value
            ...
            if _deadline_expired(deadline):
                return finish(
                    (
                        None,
                        "timed_out",
                        "shared deadline exhausted while verifying SCIP snapshot",
                        before,
                    )
                )
...
def probe(spec, root, deadline, runner) -> ProviderProbe:
    ...
    if _VERSION.fullmatch(version) is None:
        return ProviderProbe(
            ...
            detail="SCIP reader 0.6.x is required",
            repo_root=resolved,
        )
    for arguments, patterns in (
        ...
        if help_result.status != "ready":
            ...
            return ProviderProbe(
                ...
                version,
                version_result.executable_sha256,
                detail=help_result.detail or "SCIP print help unavailable",
                repo_root=resolved,
            )
        ...
        if not all(re.search(pattern, help_result.stdout) for pattern in patterns):
            return ProviderProbe(
                ...
                version,
                version_result.executable_sha256,
                detail="SCIP help does not confirm read-only JSON print",
                repo_root=resolved,
            )
    ...
    if print_status != "ready" or printed is None:
        return ProviderProbe(
            ...
            version,
            version_result.executable_sha256,
            detail=print_detail or "SCIP print failed",
            repo_root=resolved,
        )
    if printed.executable_sha256 != version_result.executable_sha256:
        return ProviderProbe(
            ...
            version,
            version_result.executable_sha256,
            detail="provider executable changed before SCIP print",
            repo_root=resolved,
        )
    try:
    ...
    return ProviderProbe(
        ...
        metadata={
            ...
            "indexer": "{} {}".format(indexer["name"], indexer["version"]),
...
def query(probe, seeds, deadline, runner) -> ProviderResult:
    ...
    seed_terms = tuple(
        term
        for term in (getattr(seed, "term", None) for seed in tuple(seeds)[:16])
    ...
    for symbol, occurrences in sorted(symbols.items()):
        if seed_terms and not any(
            term in symbol or term.rsplit(".", 1)[-1].rsplit("/", 1)[-1] in symbol
        ...
        for definition_index, definition in enumerate(definitions):
            source_path, _source_range, _, source_digest, _source_excerpt = definition
            source_key = "definition:%s:%d" % (
                hashlib.sha256(symbol.encode("utf-8")).hexdigest()[:16],
                definition_index,
            )
            nodes[source_key] = {
                "key": source_key,
                "kind": _node_kind(source_path, symbol),
                "label": symbol[-256:],
            ...
            for reference_index, reference in enumerate(references):
                ...
                nodes[target_key] = {
                    "key": target_key,
                    "kind": _node_kind(target_path, symbol),
                    "label": symbol[-256:],
