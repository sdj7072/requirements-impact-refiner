...
1. The user's current request (`simple`, `balanced`, or `technical`).
2. Repository-root `.requirements-impact-refiner.json`.
3. `balanced`.

Run `python3 "$SKILL_DIR/scripts/resolve-settings.py" --project-root REPOSITORY_ROOT`. Add `--audience MODE` only for an explicit current-request override. If configuration is invalid, disclose the error and use `balanced` for that report.
