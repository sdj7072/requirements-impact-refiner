...
Use only existing IDs in links. Create `AC-###` for every critical impact. An `accepted` impact requires a linked `DEC-###`; a `resolved` impact requires evidence explaining why it no longer applies.
